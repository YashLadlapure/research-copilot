...
const PUBLICATION_TIPS = {
  lncs: [
    ...
    'Springer LNCS requires numbered sections (1 Introduction, 2 Method…).',
    'Figures and tables must have captions below and be referenced in text.',
    'References must follow Springer LNCS numbered style [1], [2], [3].',
...
export function generateRevisionPdf(exportText, revisedSections, profile, score) {
  ...
  if (revisions.length === 0) {
    ...
    // parse WHY lines from the exportText returned by backend
    const whyMap = {};
    if (exportText) {
      const blocks = exportText.split(/\[\d+\]/);
      blocks.forEach(block => {
        const secMatch = block.match(/^\s*([A-Z]+)/);
        const whyMatch = block.match(/WHY:\s*(.+)/);
        if (secMatch && whyMatch) {
          whyMap[secMatch[1].toLowerCase()] = whyMatch[1].trim();
        }
      });
    }

    revisions.forEach(([section, revisedText], idx) => {
  ...
  drawDivider(doc, y);
  y += 6;
  y = writeWrapped(doc, 'This report was generated by Research Copilot. AI was used only for language refinement — all changes were reviewed and accepted by the author. Final editorial responsibility remains with the authors.', MARGIN, y, TEXT_W, { size: 8, color: [120, 120, 120] });

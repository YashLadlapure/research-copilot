...
export async function analyzeManuscript(text, profile, sessionId = null) {
  const res = await fetch(`${BASE}/api/analyze`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ text, profile, ...(sessionId ? { sessionId } : {}) }),
  });
...
export async function fetchBonusTips(sessionId, profile) {
  const res = await fetch(`${BASE}/api/bonus-tips`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
